...
foreach ($payments as $paymentcode => $naam) {
    upd($naam, "`" . $config['table_prefix'] . "payment_processors` SET `processor` = '" . $naam . "', `processor_script` = '" . strtolower($paymentcode) . ".php', `admin_template` = '" . strtolower($paymentcode) . ".tpl', `processor_template` = 'views/orders/components/payments/" . strtolower($paymentcode) . ".tpl', `callback` = 'Y', `type` = 'P'", $config);
}
